...
if len(active_keywords) > 0 or len(selected_locations_filter) > 0:
    ...
    if not df_raw.empty:
        ...
        if matched_candidates:
            ...
            # Initialize checkboxes in background session state
            for k in all_keys:
                if k not in st.session_state: st.session_state[k] = False
            if master_key not in st.session_state: st.session_state[master_key] = False

            # ✅ CALLBACK 1: Toggles all individual row checkboxes when Master changes
            def toggle_all_rows():
                for k in all_keys:
                    st.session_state[k] = st.session_state[master_key]

            # ✅ CALLBACK 2: Safely unchecks Master checkbox behind the scenes when any row changes
            ...
            for c in matched_candidates:
                ...
                with r0: 
                    # ✅ PRISTINE BINDING: Invokes state sync on change cleanly with zero instantiation conflicts!
